...
bool Avatar::Posicion(Vector2f pos, b2Body* body)
{
	b2Vec2 mousePos(pos.x, pos.y);
	if (!body) {
		return false; // O lanza una excepción, dependiendo de tu manejo de errores
	}
	for (b2Fixture* f = body->GetFixtureList(); f; f = f->GetNext())
	{
		if (f->TestPoint(mousePos))
		{
			return true; // Si el punto está dentro de alguna fixture, retornamos true inmediatamente
		}
	}
	return false; // Si el bucle termina sin encontrar el punto, retornamos false
}